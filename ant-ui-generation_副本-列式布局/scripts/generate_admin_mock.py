...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate an Ant-style query/list admin page in pure HTML/CSS. "
            "List is rendered as column-based independent div columns."
        )
    )
    ...
    parser.add_argument(
        "--fields",
        default=None,
        help="Comma-separated list fields. If set, use exactly as provided.",
    )
    parser.add_argument("--rows", type=int, default=10, help="Mock row count")
    parser.add_argument(
        "--include-error-state",
        action="store_true",
        help="Render first query control in error style (when validation is requested).",
    )
    parser.add_argument(
        "--show-list-header",
        action="store_true",
        help="Show list title/count header above the column table.",
    )
    parser.add_argument(
        "--show-vertical-lines",
        action="store_true",
        help="Show vertical divider lines between columns.",
    )
...
def _read_template() -> str:
    path = Path(__file__).resolve().parent.parent / "assets" / "admin-states-template.html"
...
def _cell_class(field: str) -> str:
    if _is_tags_field(field):
        return "col-cell col-cell-tags"
    if _is_action_field(field):
        return "col-cell col-cell-action"
    return "col-cell col-cell-text"


def _build_list_columns(fields: list[str], rows_data: list[dict[str, object]]) -> str:
    name_field = _resolve_name_field(fields)
    columns: list[str] = []

    for column_index, field in enumerate(fields):
        safe_field = html.escape(field)
        cell_class = _cell_class(field)
        cells: list[str] = []
        for row_index, row in enumerate(rows_data):
            if _is_tags_field(field):
                ...
                if name_field:
                    raw = row.get(name_field, "Record")
                    name_value = str(raw)
                else:
                    name_value = f"Record {row_index + 1}"
                cell_content = _render_action(name_value)
            else:
                cell_content = html.escape(str(row.get(field, "")))
            cells.append(
                f'<div class="{cell_class}" data-row="{row_index}" data-col="{column_index}">{cell_content}</div>'
            )

        columns.append(
            (
                '<div class="column">'
                f'<div class="col-head">{safe_field}</div>'
                f"{''.join(cells)}"
                '</div>'
            )
        )
...
def _render(
    ...
    *,
    ...
    rendered = (
        ...
        .replace("{{LIST_COLUMNS}}", list_columns_html)
...
def main() -> None:
    ...
    list_columns_html = _build_list_columns(fields, rows_data)
    ...
    html_text = _render(
        ...
        list_columns_html=list_columns_html,
        rows=args.rows,
